    ...
    for term in tqdm(os.getenv("TERMS").split(",")):
        ...
        for row in rows:
            ...
            if len(th) != 0:
                ...
                credits = getContent(td[6]).replace("/", "-").split("-")
                credit_min = float(credits[0])
                if len(credits) > 1:
                    credit_max = float(credits[1])
                else:
                ...
                section_data = {
                    ...
                    "xlCap": int(getContent(td[16])),
                    "xlAct": int(getContent(td[17])),
            ...
            for course in dept["courses"]:
                for section in course["sections"]:
                    ...
                    for time in section["timeslots"]:
                        for day in filter(lambda day: day in day_offsets, time["days"]):
