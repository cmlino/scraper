...
def get_prereq_string(term, crn):
    r = requests.get(
        f"https://sis.rpi.edu/rss/bwckschd.p_disp_detail_sched?term_in={term}&crn_in={crn}"
    )
...
if __name__ == "__main__":
    ...
    for semester in tqdm(os.listdir("data")):
        ...
        crns = []
        with open(f"data/{semester}/courses.json", "r") as json_file:
        ...
        for crn in crns:
            try:
                prerequisites[crn] = get_prereq_string(semester, crn)
            except:
                prerequisites[crn] = {}
                print(f"{semester} {crn}")

        with open(f"data/{semester}/prerequisites.json", "w") as outfile:
